...
import { collectTop10News } from '@/lib/scraping'
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('TOP 10 뉴스 수집 시작...', { logId })
    
    // 1. TOP 10 뉴스 스크래핑 실행
    const scrapingResult = await collectTop10News()
    ...
    for (const article of scrapingResult.articles) {
      try {
        ...
        if (error) {
          console.error('TOP 10 기사 저장 실패:', error)
        ...
        console.error('TOP 10 기사 처리 중 오류:', error)
    ...
    console.log(`TOP 10 수집 완료: ${savedArticles.length}개 저장, ${errors.length}개 오류`)
    
    return NextResponse.json({
      success: true,
      message: `TOP 10 뉴스 수집 완료: ${savedArticles.length}개 저장`,
      data: {
    ...
    console.error('TOP 10 뉴스 수집 API 오류:', error)
    ...
    return NextResponse.json(
      {
        success: false,
        message: 'TOP 10 뉴스 수집 중 오류가 발생했습니다',
